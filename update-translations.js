...
const newKeys = {
    "보완 서류 제출이 필요합니다": {
        ...
        "mn": "Баримт бичгийн бүрдүүлэлт шаารдлагатай",
    ...
    "정확한 환급액 산출을 위해 아래 서류를 추가로 제출해 주세요.": {
        ...
        "km": "សូមបញ្ជូនឯកសារខាងក្រោមសម្រាប់ការគណនាការបង្វិលប្រាក់វិញឱ្យបានត្រឹមត្រូវ។",
        ...
        "my": "တိကျသောပြန်အမ်းငွေတွက်ချက်မှုအတွက် အောက်ပါစာရွက်စာตမ်းများကို ထပ်မံတင်ပြပါ။",
        "ne": "सही फिर्ता गणनाको लागि कृपया निम्न कागजातहरू पेश गर्नुहोस्।",
        "si": "නිවැරදි ආපසු ගෙවීමේ ගණනය කිරීම සඳහා කරුණาකර පහත ලේඛන ඉදිරිපත් කරන්න.",
...
const dirPath = path.join(process.cwd(), 'src/lib/translations');

fs.readdirSync(dirPath).forEach(filename => {
    if (filename.endsWith('.ts') && filename !== 'index.ts' && filename !== 'config.ts') {
        const langCode = filename.replace('.ts', '');
        ...
        if (lastBraceIndex !== -1) {
            ...
            for (const [key, translations] of Object.entries(newKeys)) {
                // If key already exists, skip to avoid duplicates
                if (content.includes(`"${key}":`)) continue;
                
                const val = translations[langCode] || translations['en'] || key;
                const escapedVal = val.replace(/"/g, '\\"');
                insertContent += `  "${key}": "${escapedVal}",\n`;
            }
            
            const newContent = content.slice(0, lastBraceIndex) + insertContent + content.slice(lastBraceIndex);
